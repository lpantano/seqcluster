...
obj <- IsomirDataSeqFromFiles(files = files, design = design , header = T, cov = 0)
```
...
obj_mirdeep <- IsomirDataSeqFromFiles(files = files, design = design , header = T)
```
...
print_out = function(dss, rlog=NULL, res=NULL, prefix="standard"){
  plotDispEsts(dss)
  if ( is.null(res) )
    res = results(dss)
  if ( is.null(rlog))
...
dss = DESeqDataSetFromMatrix(countData = counts[rowSums(counts>0)>3,],
                             ...
                             design = ~ condition)
dss = DESeq(dss)
all_results = handle_deseq2(dss, design, condition, "mirna" )
...
dss = DESeq(dss)
all_results = handle_deseq2(ddss_mirdeep2, design, condition, "mirdeep2")
...
dss = DESeq(dss)
all_results = handle_deseq2(dss_iso, design, condition, "isomirs")
...
dss = DESeq(dss)
all_results = handle_deseq2(dss_clus, design, condition, "clusters")
